...
#[derive(Debug, Validate, Deserialize, utoipa::ToSchema, utoipa::ToResponse)]
...
impl ActiveModel {
    ...
    pub async fn set_email_verification_sent(
        ...
        self.email_verification_sent_at = ActiveValue::set(Some(Utc::now().naive_utc()));
    ...
    pub async fn create_magic_link(mut self, db: &DatabaseConnection) -> ModelResult<Model> {
        ...
        let expired = Utc::now() + Duration::minutes(MAGIC_LINK_EXPIRATION_MIN.into());

        self.magic_link_token = ActiveValue::set(Some(random_str));
        self.magic_link_expiration = ActiveValue::set(Some(expired.naive_utc()));
        self.update(db).await.map_err(ModelError::from)
    }
